            ...
            <div class="card shadow-sm border-0 mb-4">
                <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                    ...
                    <div>
                        ...
                        <a href="<?php echo URLROOT; ?>/gs" class="btn btn-sm btn-outline-secondary"><i class="fas fa-arrow-left"></i> Back</a>
                    </div>
                </div>
                <div class="card-body">
                    <div class="row mb-3">
                    ...
                    <h5 class="mb-3"><i class="fas fa-user-tie"></i> Government Secretary Decision</h5>
                    <?php if ($data['complaint']->current_role_id == 3): ?>
                        <form action="<?php echo URLROOT; ?>/gs/approve/<?php echo $data['complaint']->id; ?>" method="post" id="decisionForm">
                            <div class="mb-3">
                            ...
                            <div class="d-flex gap-2 justify-content-end">
                                <button type="button" class="btn btn-danger" onclick="submitDecision('reject')">
                                    <i class="fas fa-times-circle"></i> Reject (Return to AO)
                                </button>
                                <button type="button" class="btn btn-success" onclick="submitDecision('approve')">
                                    <i class="fas fa-check-circle"></i> Approve (Forward to Subject & Governor)
                                </button>
                            </div>
                        </form>
                    <?php else: ?>
...
function submitDecision(action) {
    ...
    if (action === 'reject') {
        ...
        form.action = '<?php echo URLROOT; ?>/gs/reject/<?php echo $data['complaint']->id; ?>';
        Swal.fire({
            title: 'Are you sure?',
            text: 'Are you sure you want to REJECT this complaint and return it to the Administrative Officer?',
            icon: 'warning',
        ...
        form.action = '<?php echo URLROOT; ?>/gs/approve/<?php echo $data['complaint']->id; ?>';
        Swal.fire({
            title: 'Are you sure?',
            text: 'Are you sure you want to APPROVE this complaint and forward it to the Chief Clerk for final processing?',
            icon: 'question',
            showCancelButton: true,
